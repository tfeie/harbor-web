...
public class UserCommentMQSend {

	private static final Logger LOG = Logger.getLogger(UserCommentMQSend.class);

	public static void sendMQ(DoBeComment body) {
		MNSClient client = MNSFactory.getMNSClient();
		try {
			body.setMqId(UUIDUtil.genId32());
			body.setMqType(MQType.MQ_HY_BE_COMMENT.getValue());
			CloudQueue queue = client.getQueueRef(GlobalSettings.getCommentQueueName());
			...
			LOG.error("be comment message put in Queue error", se);
		} catch (Exception e) {
	...
	public static void sendMQ(DoGoComment body) {
		MNSClient client = MNSFactory.getMNSClient();
		try {
			body.setMqId(UUIDUtil.genId32());
			body.setMqType(MQType.MQ_HY_GO_COMMENT.getValue());
			CloudQueue queue = client.getQueueRef(GlobalSettings.getCommentQueueName());
			Message message = new Message();
			message.setMessageBody(JSONObject.toJSONString(body));
			queue.putMessage(message);
			...
			LOG.error("go comment message put in Queue error", se);
		} catch (Exception e) {
			LOG.error("Unknown exception happened!", e);
		}
		client.close();
